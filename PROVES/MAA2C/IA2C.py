...
# AQUEST MAA2C -> AGENTS INDEPENDENTS
#- Each agent: own actor-critic network, local observations and rewards.
# TODO: SHARED PARAMETERS ENTRE AGENTS 

NUM_AGENTS = 2
ENV_NAME = "lbforaging:Foraging-15x15-2p-5f-v3"  
LR = 1e-3
...
SAVE_DIR = "checkpoints_ind_prova"
SAVE_INTERVAL = 5000 
...
class ActorCritic(nn.Module):
    def __init__(self, obs_dim, act_dim):
        super().__init__()
        self.shared = nn.Sequential(   #### QUAN 8X8-2P-3F
...
def train():
    ...
    for episode in range(MAX_EPISODES):
        ...
        total_rewards.append(episode_rewards.sum())  ##SUMA PQ SINO EL GRAFIC CANVIA AMB EL NOMBRE D'AGENTS
        wandb.log({"steps_per_episode": steps_per_episode}, step=episode)
        wandb.log({'mean_steps_1000': np.mean(total_steps_per_episode[-1000:])}, step=episode)
        wandb.log({"avg_reward_1000": np.mean(total_rewards[-1000:])}, step=episode)
        wandb.log({"episode_reward": episode_rewards.sum(), "episode": episode})
        # wandb.log({"total loss": loss.item()})

        if episode % 10 == 0:
            print(f"Episode {episode}, Steps: {steps_per_episode}, Reward: {episode_rewards}")
        if episode % 100 == 0:
            print(f"Episode {episode}, Total Reward (sum): {np.mean(total_rewards[-100:])}")
